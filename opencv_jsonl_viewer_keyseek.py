...
class PyAvVideoFrameReader:
    ...
    def open(self) -> None:
        ...
        # 允许 FFmpeg 库内部用多线程解码。不同 PyAV 版本字段略有差异，所以这里做兼容。
        try:
    ...
    def seek_to_keyframe(self, keyframe_index: int) -> None:
        ...
        if keyframe_index <= 0:
            # HLS/m3u8 在起始 PTS 上调用 seek 可能会返回 Operation not permitted。
            # 第一段本来就不需要 seek，重新打开后直接从头解码最稳。
            self.open()
            return

        key_pts = self.annotation.pts_values[keyframe_index]
        try:
            # 指定 stream 后，timestamp 使用该视频流自己的 time_base；这里就是 m3u8/ts 里的原始 PTS。
            self.container.seek(key_pts, stream=self.stream, backward=True, any_frame=False)
        except Exception:
            # 少数 HLS/TS seek 状态会变脏，重新打开后再 seek 一次。
            self.open()
            self.container.seek(key_pts, stream=self.stream, backward=True, any_frame=False)
...
class OpenCvJsonlViewer:
    def __init__(
        ...
        self.root.title("OpenCV JSONL 检测结果查看器 - PyAV 关键帧 Seek 版")
    ...
    def build_layout(self) -> None:
        ...
        self.threshold_row.grid(row=3, column=0, sticky="ew", pady=(6, 0))
        ...
        ttk.Button(row1, text="选择视频目录", command=self.choose_video_dir).grid(row=0, column=14, padx=(18, 4))
    ...
    def choose_video_dir(self) -> None:
        selected = filedialog.askdirectory(
            title="选择视频目录",
            initialdir=str(self.video_dir if self.video_dir.is_dir() else PROJECT_DIR),
        )
        if not selected:
            return
        ...
        self.video_dir = Path(selected).expanduser().resolve()
    ...
    def ensure_video_keyframe_index(self, camera_id: int, playlist_path: Path) -> bool:
        if camera_id in self.video_keyframe_indexes:
            return True
        try:
            annotation = self.store.cameras[camera_id]
            self.video_keyframe_indexes[camera_id] = build_video_keyframe_index(playlist_path, annotation)
            return True
        except Exception as exc:
            messagebox.showerror("关键帧索引失败", f"构建相机 {camera_id} 的关键帧索引失败：\n{exc}")
            return False
    ...
    def read_video_frame(self, frame_index: int) -> Any | None:
        ...
        # 统一从目标前最近关键帧用 PyAV 解出一小段缓存。
        # PyAV 底层仍是 FFmpeg 库，但不会反复启动 ffmpeg.exe 外部进程。
    ...
    def render_frame(self, frame: Any) -> None:
        annotation = self.get_annotation()
        video_pts = annotation.pts_values[self.current_index]
        if self.is_video_mode() and self.json_offset_frames != 0:
            self.json_offset_frames = 0
            self.json_offset_var.set("0")
